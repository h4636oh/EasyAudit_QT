...
if [[ -n "$var_log_partition" ]]; then
    # Add noexec to the fourth field in /etc/fstab for /var/log partition
    echo "Editing /etc/fstab to add noexec option for /var/log partition..."
    sudo sed -i 's|\(/var/log.*defaults.*\)|\1,noexec|' /etc/fstab
    ...
    echo "The /var/log partition has been remounted with the noexec option."
else
