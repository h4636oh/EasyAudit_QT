...
# Check if UFW is installed and if so, get its config file
l_ufwscf="$([ -f /etc/default/ufw ] && awk -F= '/^\s*IPT_SYSCTL=/ {print $2}' /etc/default/ufw)"

# Check if necessary utilities are available
...
# Function to check kernel parameters in running configuration and files
kernel_parameter_chk() {
    ...
    # Assess output from files and generate result
    if (( ${#A_out[@]} > 0 )); then
